...
if (!token) {
  console.error(
    "No Supabase access token. Run `npx supabase login` or add SUPABASE_ACCESS_TOKEN to .env.local",
  );
  process.exit(1);
}
...
console.log("OK: signal_imports migration applied via Supabase Management API");
